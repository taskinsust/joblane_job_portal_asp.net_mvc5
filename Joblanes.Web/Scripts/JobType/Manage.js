$(document).ready(function () {
    function dataTableRender() {
        ...
        $('#DataGrid').dataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: $("body").attr("data-project-root") + "JobType/JobTypeList",
                type: 'POST',
                data: function (d) {
                    d.name = $('#Name').val();
                    d.status = $('#Status').val();
    ...
    $(document).on("click", ".glyphicon-trash", function () {
        ...
        if (id != "") {
            bootbox.deleteConfirm("<h3>Are you sure you want to delete this <span class='confirm-message'>" + name + "</span> Job Type?</h3>", function (result) {
                if (result) {
                    $.ajax({
                        type: "post",
                        url: $("body").attr("data-project-root") + "JobType/Delete",
                        cache: false,
                        async: true,
                        data: { "id": id },
                        beforeSend: function () {
                        ...
                        error: function (result) {
                            var errorMessage = '<div class="alert alert-danger"><a class="close" data-dismiss="alert">×</a>Job Type Delete Failed</div>';
                            $('.customMessage').append(errorMessage);
                        }
                    });
